...
Quick guide for AI agents to work effectively in this repo. Focus on what exists and how components talk to each other.
...
Note: Two task systems exist in the repo for the MVP — Django DB (primary, used by web) and assistant JSON/in-memory (used by assistant/mobile). They are intentionally not synced.
...
- Frontend
  - cd `frontend`; `npm install`; `npm start` (uses `REACT_APP_API_URL` if set).
- Smoke test API
...
- Docs: `docs/api/{overview.md,auth.md,tasks.md}`, `docs/backend/runbook.md`, `docs/architecture.md`

## For AI changes
- Pick the correct boundary first (Django vs assistant). Don’t cross-write between systems unless explicitly required.
- If you change public API shapes or behavior, update `docs/api/*` and confirm `frontend/src/services/apiService.js` expectations.
